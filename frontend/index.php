...
//设置默认属性
$currentPage = isset($_GET['page']) ? $_GET['page'] : 1;		//分页需要
$type = isset($_GET['type']) ? $_GET['type'] : 'all';			//分类分页需要

//分类分页显示文章数据
$everyPageShow = 8;
if($type == 'all'){
	$sql = 'select title, content, create_time from article limit '.($currentPage-1)*$everyPageShow.','.$everyPageShow;
	$stmt = $conn->prepare($sql);
	$stmt->execute();
	$res = $stmt->fetchAll(PDO::FETCH_ASSOC);
	$totalDataNum = (int)$conn->query('select count(*) from article')->fetch()[0];
}elseif($type=='prose' || $type=='webtec'){
	$sql = 'select title, content, create_time from article where art_type=:type limit '.($currentPage-1)*$everyPageShow.','.$everyPageShow;
	$stmt = $conn->prepare($sql);
	$stmt->bindParam(':type', $type);
	$stmt->execute();
	$res = $stmt->fetchAll(PDO::FETCH_ASSOC);
	$totalDataNum = (int)$conn->query('select count(*) from article where art_type="'.$type.'"')->fetch()[0];
}else{
	echo '请不要尝试非法操作';
	exit;
}
...
<body>
	...
	<div class='frame'>
		<div class='nav'>
			...
			<br>
			<div class='barframe'>			<!-- nav_bar固定后由父元素占位 -->
				<div class='nav_bar'>
					<div class='bar'>
				...
				if($type == 'all'){
					...
					if($totalPageNum < $pagination){
						for($i=1; $i<=$totalPageNum; $i++){
							echo '<a href="/index/all/'.$i.'.html"><span>'.$i.'</span></a>';
						}
					}else{
						if($currentPage+2 <= $pagination){
							...
							echo '…';
							echo '<a href="/index/all/'.$totalPageNum.'.html"><span>'.$totalPageNum.'</span></a>';
						}elseif(($pagination - $currentPage) <= 1){
							echo '<a href="/index/all/1.html"><span>1</span></a>';
							echo '…';
					...
					if($totalPageNum < $pagination){
						for($i=1; $i<=$totalPageNum; $i++){
							echo '<a href="/index/'.$type.'/'.$i.'.html"><span>'.$i.'</span></a>';
						}
					}else{
						if($currentPage+2 <= $pagination){
							for($i=1; $i<=$pagination; $i++){
							...
							echo '…';
							echo '<a href="/index/'.$type.'/'.$totalPageNum.'.html"><span>'.$totalPageNum.'</span></a>';
						}else{
							echo '<a href="/index/'.$type.'/1.html"><span>1</span></a>';
							echo '…';
							for($i=$currentPage-floor($pagination/2); $i<$currentPage; $i++){
								echo '<a href="/index/'.$type.'/'.$i.'.html"><span>'.$i.'</span></a>';
							}
