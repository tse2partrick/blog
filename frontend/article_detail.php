...
<body>
	<?php 
		...
		//回复评论后跳转到相对应的评论的最后回复页，如果相应评论没有回复的话，设第一次回复为第一页
			for($i=0; $i<count($allReplyNumArr); $i++){
				if(!empty($allReplyNumArr[$i])){
					if($allReplyNumArr[$i] % 8 != 0){
						echo '<input type="hidden" id="replyPage_'.$i.'" value="'.$replyTotalPageNum[$i].'">';
					}else{
						echo '<input type="hidden" id="replyPage_'.$i.'" value="'.($replyTotalPageNum[$i]+1).'">';


						//echo $allReplyNumArr[$i]%8;  偶然出了两次错误，跳到了最后一页+1导致无法显示回复
					}
				}else{
		...
		//评论是每页显示10条，如果发表新评论的时候旧评论满10条就跳到第二页
		if(count($allCommentArr) <= 9){
			$pageValue = $totalPageNum != 0 ? $totalPageNum : 1;		//如果文章没有评论，总页数设为1
		}else{
			...
			<div class='comment_menu'>
				<?php
					for($i=0; $i<count($allCommentArr); $i++){
						...
						$cmtFloor = ($current_page-1)*10 + $i + 1;
						echo '<div class="menu_frame" id="'.$i.'">';
							echo '<div class="menu_face"><img class="menu_face_img" src="'.$allCommentArr[$i]['face'].'" /></div>';
							echo '<div class="menu_three">';
								echo '<div class="menu_three_name">'.$ip.'<span class="menu_three_floor">'.($cmtFloor).'楼</span></div>';
								...
								//有内容回复框DIV开始
									...
									foreach($everyIdReplyDataArr[$i] as $k => $v){
											...
											echo '<div class="replyFace"><img class="replyFace_img" src="'.$v['reply_face'].'" /></div>';
												echo '<div class="replyContentFrame">';
													echo '<div class="replyName" >'.$v['reply_name'].'</div>';
