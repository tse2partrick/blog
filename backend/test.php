...
if(!empty($hasComment)){
	...
	for($i=0; $i<count($hasComment); $i++){
		$hasReply = $conn->query('select id from cmt_reply where comment_id='.$hasComment[$i]['id'].'')->fetchAll(PDO::FETCH_ASSOC);

		//如果有回复就删除回复
		if($hasReply){
			$result = $conn->exec('delete from cmt_reply where comment_id = '.$hasComment[$i]['id'].'');
		}
